export default class InhabitantModel {
  ...
  createInhabitantByRoleandProvinceId(provinceId, roleId) {
    return new Promise((resolve, reject) => {
      const query =
        'INSERT INTO inhabitant(health, attack, defense, image, province_id, role_id) VALUES (100, 75, 50, "legionary.jpg", ?, ?);';
      const values = [provinceId, roleId];
      this.connection.execute(query, values, (error, result, fields) => {
        if (error) {
          reject(error);
        } else {
  ...
  selectInhabitantRoleByDescription(description) {
    return new Promise((resolve, reject) => {
      const query =
        'select role.name, role.id, role.description from role where role.description =?;';
      const values = [description];
      this.connection.execute(query, values, (error, result) => {
        console.log(query, 'requete role');
        if (error) {
          reject(error);
        } else {
          console.log(result[0], 'resultat');
          resolve(result[0]);
        }
      });
    });
